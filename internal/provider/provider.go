	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			"api_url": schema.StringAttribute{
				Description: "Base URL of the Ace Cloud API. Set explicitly via the provider block or the `ACECLOUD_API_URL` environment variable. If neither is set, the provider reads `api_base_url` from the ace-cli config file (`~/.ace/config.json` by default).",
				Optional:    true,
			},
			...
			"ace_config_path": schema.StringAttribute{
				Description: "Path to the ace-cli configuration file. Defaults to `~/.ace/config.json`. When no explicit API key is configured, the provider reads credentials from this file as the lowest-priority authentication method. Note: `region` and `project_id` are not read from this file — set them explicitly.",
				Optional:    true,
			},
		},
	}
...
func (p *AceCloudProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	// If api_url was not set explicitly, fall back to the ace-cli config file.
	// This is the one carve-out where ace-cli config can supply a non-credential
	// value — useful for local development where the same config file already
	// has the endpoint configured. Region and project_id are NOT read from the
	// CLI config and must come from the provider block or environment.
	if apiURL == "" {
	...
	// region and project_id are Required at the schema level, so by the time we
	// reach here the framework has guaranteed both are non-empty. Use them as-is.
	...
	if apiURL == "" {
		resp.Diagnostics.AddError(
			"Missing API URL",
			"`api_url` is required. Set it in the provider configuration, via the "+
				"`ACECLOUD_API_URL` environment variable, or have it populated by your "+
				"ace-cli config file at "+aceConfigPath+".",
		)
		return
	}
	...
	if err := c.ValidateToken(ctx); err != nil {
		resp.Diagnostics.AddError(
			...
			fmt.Sprintf("The provided credentials could not be validated (%s method): %s\n\n"+
				...
				"If using ace-cli config, try re-authenticating: ace auth login-api-key", result.Method, err),
		)
		return
	}
