...
// stringOrNull returns a typed string value, or null if empty.
...
type poolCreateRequest struct {
	...
	SessionPersistence *sessionPersistenceAPIObject `json:"session_persistence,omitempty"`
}

type sessionPersistenceAPIObject struct {
...
type poolAPIResponse struct {
	...
	SessionPersistence *sessionPersistenceAPIObject `json:"session_persistence"`
...
func buildCreateRequest(ctx context.Context, plan *lbPoolResourceModel) poolCreateRequest {
	...
	if !plan.SessionPersistence.IsNull() && !plan.SessionPersistence.IsUnknown() {
		var sp sessionPersistenceModel
		plan.SessionPersistence.As(ctx, &sp, basetypes.ObjectAsOptions{})
		body.SessionPersistence = &sessionPersistenceAPIObject{
			Type:       sp.Type.ValueString(),
			CookieName: sp.CookieName.ValueString(),
		}
	}
	return body
}

func mapAPIResponseToState(ctx context.Context, model *lbPoolResourceModel, apiResp *poolAPIResponse) {
	...
	model.TLSEnabled = types.BoolValue(apiResp.TLSEnabled)
	...
	// session_persistence: only populate if the user originally set it (avoid
	// drift when API returns null for an unset value).
	if !model.SessionPersistence.IsNull() && !model.SessionPersistence.IsUnknown() {
		if apiResp.SessionPersistence != nil {
			obj, _ := types.ObjectValue(sessionPersistenceAttrTypes(), map[string]attr.Value{
				"type":        types.StringValue(apiResp.SessionPersistence.Type),
				"cookie_name": stringOrNull(apiResp.SessionPersistence.CookieName),
			})
...
func (r *lbPoolResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	mapAPIResponseToState(ctx, &state, pool)
