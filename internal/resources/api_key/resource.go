...
func (r *apiKeyResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil || parsed.APIKey == "" {
		resp.Diagnostics.AddError(
			...
			"The backend did not return the API key secret. The key may have been created but is unrecoverable from Terraform state. "+
				"Run 'ace api-key list' to verify, then 'ace api-key revive <id>' to regenerate the secret.",
		)
		return
	}
