...
func (r *instanceResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	// Save the user-intended power_state BEFORE calling mapReadResponseToState
	// (which infers power_state from the just-built ACTIVE instance,
	// clobbering any non-default value the user set in their HCL config).
	...
	// Restore user intent so the action checks below see the configured value.
...
func (r *instanceResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// 3. Power state — PUT /cloud/instances/{id}/power?value=ON|OFF.
	...
	// 4. Generic PUT for name + description.
...
//
// Note: the backend only supports resizing to a larger flavor; attempting
// to downgrade returns "Resource flavor cannot be downgraded" and the
// provider surfaces that error verbatim.
func (r *instanceResource) resizeInstance(ctx context.Context, instanceID, newFlavorID string) error {
	...
	// actually changed; if not, surface a clear error so the caller doesn't
	// believe a successful resize happened when the backend silently no-op'd.
	if verifyResult.Status == "ACTIVE" {
		...
		if inst != nil && inst.FlavorID != "" && inst.FlavorID != newFlavorID {
			return fmt.Errorf("instance returned to ACTIVE on the original flavor — backend did not perform the resize. Contact platform support if this persists")
		}
...
func (r *instanceResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	// Locked instances reject delete. Fetch live lock state (it may have been
	// toggled out-of-band) and unlock first if needed.
	if apiResp, err := r.client.Get(ctx, fmt.Sprintf("/cloud/instances/%s", id), nil); err == nil {
		if inst, perr := parseInstanceData(apiResp.Data); perr == nil && inst.Locked {
...
func mapReadResponseToState(ctx context.Context, instance *readInstanceResponse, state *instanceResourceModel) diag.Diagnostics {
	...
	// power_state derived from instance status.
