...
func (d *flavorsDataSource) Read(ctx context.Context, _ datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	// The API response is the list of flavors. Note: the backend uses
	// camelCase inside `extra_specs` (e.g. `isHourly`, `requestedQuantity`)
	// while top-level fields are snake_case / lowercase. Earlier versions of
	// this struct used snake_case tags for the extra_specs fields, so those
	// fields silently came back as their zero values.
	var raw []struct {
		ID         string  `json:"id"`
	...
	state := flavorsDataSourceModel{Flavors: []flavorModel{}}
	for _, f := range raw {
		ram := f.RAM
		if ram == 0 && f.Memory > 0 {
			ram = f.Memory
