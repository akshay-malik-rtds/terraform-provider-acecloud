...
type vpcModel struct {
	...
	RouterExternal types.Bool   `tfsdk:"router_external"`
			...
			"vpcs": schema.ListNestedAttribute{
				...
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						...
						"router_external": schema.BoolAttribute{
							Description: "Whether the VPC is an external (public) network.",
							Computed:    true,
						},
						"shared": schema.BoolAttribute{
							Description: "Whether the VPC is shared across projects.",
							Computed:    true,
						},
					},
				},
...
func (d *vpcsDataSource) Read(ctx context.Context, _ datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	state := vpcsDataSourceModel{VPCs: []vpcModel{}}
	for _, v := range raw {
		state.VPCs = append(state.VPCs, vpcModel{
			...
			MTU:            types.Int64Value(v.MTU),
			RouterExternal: types.BoolValue(v.RouterExternal),
