...
Data source correctness release. No state migration required.

**Fixed silently-broken fields (no action required):**

- `data.acecloud_flavors.flavors[*].gpu_count` and `is_hourly` now report actual values (were always `0` / `false`).
- `data.acecloud_images.images[*].disk_format`, `container_format`, and `virtual_size` now report actual values (were always `""` / `0`).
- All list data sources now return `[]` rather than `null` when empty.

**Schema change on `data.acecloud_vpcs` (action required if you used `router_id`):**

- **Removed:** `vpcs[*].router_id` — this field always returned `""` because it was wired to a non-existent upstream key. Remove any references to it.
- **Added:** `vpcs[*].mtu`, `vpcs[*].router_external` (bool — true for public/external networks), `vpcs[*].shared` (bool — true for cross-project shared networks).
