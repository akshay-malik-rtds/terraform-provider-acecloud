...
- **Clarified:** the `acecloud_api_key` resource documentation now states accurately that key creation requires user-session authentication, so `terraform apply` against this resource fails under API-key auth. Use `terraform import` after creating a key from the console.
