...
1. API key (api_key_id + api_key_secret + api_key_service_name, or the matching ACECLOUD_API_KEY_* environment variables) — recommended for automation and CI/CD
2. ace-cli config file at ~/.ace/config.json (created by 'ace auth login' or 'ace auth login-api-key') — for zero-config local development
---
...
The provider supports two authentication methods, tried in this order:

1. **API key** (recommended for everything) — long-lived credentials suitable for both automation and interactive use. Set `api_key_id`, `api_key_secret`, and `api_key_service_name` in the provider block, or use the environment variables `ACECLOUD_API_KEY_ID`, `ACECLOUD_API_KEY_SECRET`, and `ACECLOUD_API_KEY_SERVICE_NAME`. Create a key with `ace api-key create --service-name <name>`.

2. **`ace-cli` config** (zero-config local development) — reads credentials and `api_url` from `~/.ace/config.json`, the file written by `ace auth login-api-key`. Useful when you're already authenticated through the CLI. Note: `region` and `project_id` are **not** read from the CLI config — they must be set explicitly in the provider block or via environment variables.

If both methods are configured, the API key in the provider block wins.

> **Why only these two?** Static JWT tokens expire in 24 hours and are unsuitable for unattended automation. Email/password login transmits raw user credentials, can't handle 2FA accounts, and ties Terraform state lifetime to a personal user account. API keys are the right primitive for infrastructure-as-code: scoped, rotatable, revocable, and tied to a service identity rather than a person.

## Region and Project ID

The `region` and `project_id` arguments are **required** for every provider configuration. Set them in the provider block or via the `ACECLOUD_REGION` / `ACECLOUD_PROJECT_ID` environment variables. Terraform will fail at plan time if either is missing — there is no auto-selection.

This deliberate constraint keeps `terraform plan` deterministic: the same configuration produces the same plan on every machine, regardless of which account or CLI session ran it.

### `api_url` is the one exception

`api_url` is the only required-at-runtime field that the provider can fill in for you. If you don't pass it explicitly (and `ACECLOUD_API_URL` is not set), the provider falls back to the `api_base_url` value in `~/.ace/config.json` written by `ace auth login`. This keeps the zero-config local development path working when you've already authenticated through the CLI.

For CI/CD and any non-interactive use, set `api_url` explicitly (or via `ACECLOUD_API_URL`) — don't rely on the CLI config fallback.
...
| `ACECLOUD_CONFIG_PATH` | `ace_config_path` |
...
- `ace_config_path` (String) Path to the ace-cli configuration file. Defaults to `~/.ace/config.json`. When no explicit API key is configured, the provider reads credentials from this file as the lowest-priority authentication method. Note: `region` and `project_id` are not read from this file — set them explicitly.
...
- `api_url` (String) Base URL of the Ace Cloud API. Set explicitly via the provider block or the `ACECLOUD_API_URL` environment variable. If neither is set, the provider reads `api_base_url` from the ace-cli config file (`~/.ace/config.json` by default).
