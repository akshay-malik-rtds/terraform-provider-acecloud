...
`api_url` is optional. When unset, the provider targets the production endpoint `https://customer.acecloud.ai/api/v1/`. Override it only when targeting a non-production endpoint, either through the provider block or the `ACECLOUD_API_URL` environment variable.
