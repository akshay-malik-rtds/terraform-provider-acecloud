...
### Pattern: one Terraform configuration, two regions
...
# Aliased provider — used by resources that opt into Mumbai.
provider "acecloud" {
  alias                = "mumbai"
  api_key_id           = var.acecloud_api_key_id
  api_key_secret       = var.acecloud_api_key_secret
...
### Pattern: one configuration, two projects (same region)
