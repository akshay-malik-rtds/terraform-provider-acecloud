...
- `acecloud_api_key` — IAM API keys (long-lived programmatic credentials)
...
**Authentication:**
- API key (`api_key_id` + `api_key_secret`) — long-lived credentials for automation
- Static JWT token (`api_token`)
- Email and password login
- `ace-cli` config (`~/.ace/config.json`) — supports both API keys and JWT tokens
- Environment variables for all credentials (`ACECLOUD_API_KEY_ID`, `ACECLOUD_API_KEY_SECRET`, `ACECLOUD_API_KEY_SERVICE_NAME`, `ACECLOUD_API_TOKEN`, `ACECLOUD_EMAIL`, `ACECLOUD_PASSWORD`, etc.)
