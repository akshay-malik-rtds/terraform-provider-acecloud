...
The provider supports four authentication methods, tried in order:

1. **API key** *(recommended for automation)* — set `api_key_id`, `api_key_secret`, and `api_key_service_name` in the provider block, or export `ACECLOUD_API_KEY_ID`, `ACECLOUD_API_KEY_SECRET`, and `ACECLOUD_API_KEY_SERVICE_NAME`. The service name is required and must match the value supplied at key creation. Create an API key with `ace api-key create --service-name <name>`.
2. **Static JWT token** — set `api_token` in the provider block, or export `ACECLOUD_API_TOKEN`. Tokens expire after 24 hours; for long-running automation use an API key instead.
3. **Email and password** — set `email` and `password` in the provider block, or export `ACECLOUD_EMAIL` and `ACECLOUD_PASSWORD`. Not supported with 2FA-enabled accounts.
4. **`ace-cli` config** — run `ace auth login` or `ace auth login-api-key` to populate `~/.ace/config.json`; the provider will read it automatically.
